...
*/
exports.processFile = function(conf, sftp, dir, fileName, process) {
  ...
  .then(function(fileInfo) {
    ...
    .then(function(handle) {
      ...
      .then(function(data) {
        return sftp.readdirAsync(dir)
        .then(function(dirList) {
          if (!dirList.find(function(item) { return item.filename == conf.DoneDir})) return sftp.mkdirAsync(dir + '/' + conf.DoneDir);
        })
        .then(function() {
          return sftp.renameAsync(dir + '/' + fileInfo.filename, dir + '/' + conf.DoneDir + '/' + getCurrentTimestamp() + '-' + fileInfo.filename);
        })
        .then(function() {
          return data;
        })
